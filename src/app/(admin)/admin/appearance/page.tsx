...
import {
  ...
  updateTopBar,
  updateHeroSlides,
} from "@/modules/admin/actions"; // পাথ চেক করুন
// Components
...
export default function AppearancePage() {
  ...
  const barForm = useForm<TopBarFormValues>({
    ...
    defaultValues: { text: "", link: "", isActive: true, useSchedule: false },
  });

  const slidesForm = useForm<SlidesFormValues>({
    resolver: zodResolver(slidesFormSchema),
    defaultValues: {
      slides: [{ title: "", subtitle: "", image: "", link: "" }],
    },
  });
    ...
    async function load() {
      try {
        ...
        if (data.settings) {
          ...
          barForm.reset({
            text: data.settings.topBarText || "",
            link: data.settings.topBarLink || "",
            isActive: data.settings.topBarActive,
            // যদি ডেট থাকে, তার মানে শিডিউল অন ছিল
            useSchedule: !!(
              data.settings.topBarStart || data.settings.topBarEnd
            ),
            topBarStart: formatDate(data.settings.topBarStart),
            topBarEnd: formatDate(data.settings.topBarEnd),
          });
        }
        if (data.slides.length > 0) {
          ...
          slidesForm.reset({ slides: formattedSlides });
        }
      } catch (error) {
  ...
  const onSaveBar = async (data: TopBarFormValues) => {
    ...
    try {
      await updateTopBar({
        ...
        // লজিক: যদি useSchedule false হয়, আমরা ডেটগুলো ডিলিট করে দিব (empty string পাঠিয়ে)
        // useSchedule ফিল্ডটি সার্ভারে পাঠানোর দরকার নেই
    ...
    <div className="min-h-screen bg-gray-50/50 pb-32">
      ...
      >
        ...
        <div className="space-y-8">
          ...
          <section className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm group hover:border-indigo-200 transition-colors">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    {/* ✅ New Advanced Status Badge */}
                    <StatusBadge
                      isActive={barForm.watch("isActive") ?? false}
            ...
            <div className="space-y-6">
              ...
              {barForm.watch("useSchedule") && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-amber-50/50 border border-amber-100 rounded-xl animate-in fade-in slide-in-from-top-2">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      min={currentDateTime}
                      className="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm focus:border-amber-500 outline-none"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-gray-500 uppercase ml-1">
          ...
          {/* Hero Slider Form */}
          <section className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm group hover:border-indigo-200 transition-colors">
            {/* ... (Slider Header and Form remains same as provided in previous nice clean-up) ... */}
            {/* For brevity, keeping the logic you already have. Just ensure the StatusBadge here is removed or simplified since you wanted NO schedule for slides */}
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
